...
def apso(particle, gbest, pbest, velocity, params):
    ...
    new_velocity = np.asarray(cur_velocity) * w + c1 * r1 * (new_pbest - cur_particle) + c2 * r2 * (
                new_gbest - cur_particle)
    ...
    while pool_num > params['max_pool']:
        new_particle, new_velocity, particle_conn, velocity_conn = cut_pool(new_particle, new_velocity, particle_conn,
                                                                            velocity_conn)
        pool_num = _calculate_pool_numbers(new_particle)
...
def cpso(cur_particle, gbest, pbest, velocity, params, offsets):
    ...
    for i in range(cur_len):
        ...
        new_velocity[i] = np.asarray(velocity[i]) * w + c1 * r1 * (
                    new_pbest_binList - cur_particle_binList) + c2 * r2 * (new_gbest_binList - cur_particle_binList)
