...
class Utils(object):
        ...
        for i in range(len(particle_c)):
            ...
            for j in range(i + 1):
                if particle_c_bin[j] == '1':
                    num_output[j] += 1
        target_output_indices = [list(enumerate(num_output))[i][0] - 1 for i in range(len(num_output)) if
                                 num_output[i] == 0]
    ...
    def generate_pytorch_file(cls, particle, curr_gen, id):
        ...
        conv_begin1 = 'self.conv_begin = nn.Conv2d(%d, %d, kernel_size=3, stride=1, padding=1, bias=False)' % (
        img_channel, inchannels[0])
        ...
        for i, dimen in enumerate(particle_a):
            ...
            if 0 <= dimen <= 15:
                ...
                if conv_name not in conv_name_list:
                    ...
                    conv = '%s = BasicBlock(in_planes=%d, planes=%d, kernel_size=%d, act_func="%s", drop_connect_rate=%.1f)' % (
                    conv_name, in_channel, out_channel, kernel_sizes[num_pool], act_funcs[num_pool],
                    drop_rates[num_pool])
                    conv_list.append(conv)

        conv_end1 = 'self.conv_end = nn.Conv2d(%d, %d, kernel_size=1, stride=1, bias=False)' % (
        ...
        fully_layer_name4 = 'self.linear = nn.Linear(%d, %d)' % (
        factor * channels_end, cls.get_params('network', 'num_class'))
